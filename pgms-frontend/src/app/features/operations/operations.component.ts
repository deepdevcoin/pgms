...
import { MenuItem, PG, Role } from '../../core/models';
...
@Component({
            ...
            <div class="actions">
              @for (action of actions(); track action.label) {
                @if (action.show(row, auth.role())) {
                  <button class="icon" type="button" (click)="action.run(row)" [title]="action.label">
  ...
  styles: [`
    ...
    .table { overflow: auto; }
    .thead, .tr { display: grid; grid-template-columns: repeat(var(--cols, 6), minmax(140px, 1fr)) 120px; gap: 10px; align-items: center; min-width: 900px; }
...
export class OperationsComponent {
  ...
  constructor() {
    this.api.listPgs().subscribe({ next: pgs => this.pgs.set(pgs), error: () => undefined });
    this.resetForm();
    this.load();
  }
  ...
  load() {
    ...
    console.log('PG ID:', this.numberField('pgId'), this.pgs());
          ...
          : key === 'vacate'
            ...
            : key === 'services'
              ...
              : key === 'amenities' && this.auth.role() === 'MANAGER'
                ? this.api.createAmenitySlot({ amenityType: this.form['amenityType'], slotDate: this.form['slotDate'], startTime: this.form['startTime'], endTime: this.form['endTime'], capacity: this.numberField('capacity'), facilityName: this.form['facilityName'] })
                : key === 'sublets'
  ...
  label(col: string): string {
    return col.replace(/([A-Z])/g, ' $1').replace(/^./, c => c.toUpperCase());
  }
  ...
  pillClass(status: unknown): string {
  ...
  private moduleConfig(key: ModuleKey): ModuleConfig {
    ...
    const configs: Record<ModuleKey, ModuleConfig> = {
      payments: {
        ...
        columns: ['tenantName', 'roomNumber', 'billingMonth', 'totalDue', 'amountPaid', 'remainingAmountDue', 'fineAccrued', 'status'],
        createLabel: role === 'MANAGER' ? 'Record cash' : undefined,
        fields: role === 'MANAGER' ? [
          { key: 'tenantProfileId', label: 'Tenant profile ID', type: 'number' },
          { key: 'billingMonth', label: 'Billing month', type: 'text' },
      ...
      notices: {
        ...
        fields: role === 'OWNER' || role === 'MANAGER' ? [
          ...
          { key: 'targetType', label: 'Target', type: 'select', options: role === 'OWNER' ? ['ALL_TENANTS', 'PG_TENANTS', 'ALL_MANAGERS', 'SPECIFIC_MANAGER', 'SPECIFIC_TENANT'] : ['ALL_TENANTS', 'PG_TENANTS', 'SPECIFIC_TENANT'] },
      ...
      amenities: {
        ...
        columns: ['amenityType', 'facilityName', 'slotDate', 'startTime', 'endTime', 'capacity', 'bookingCount', 'status'],
  ...
  private moduleActions(key: ModuleKey): ActionConfig[] {
    const actions: Record<ModuleKey, ActionConfig[]> = {
      payments: [
        { label: 'Pay', icon: 'payments', show: row => this.auth.role() === 'TENANT' && row['remainingAmountDue'] > 0, run: row => this.promptAmount(amount => this.api.payRent(row['id'], amount)) },
        ...
        { label: 'Waive fine', icon: 'money_off', show: row => this.auth.role() === 'MANAGER' && row['fineAccrued'] > 0, run: row => this.promptText('Reason', reason => this.api.waiveFine(row['id'], reason)) }
      ],
      complaints: [
        ...
        { label: 'Resolve', icon: 'task_alt', show: row => this.auth.role() !== 'TENANT' && row['status'] !== 'RESOLVED', run: row => this.promptText('Resolution notes', notes => this.api.updateComplaint(row['id'], 'RESOLVED', notes)) },
      ...
      services: [
        ...
        { label: 'Rate', icon: 'star', show: row => this.auth.role() === 'TENANT' && row['status'] === 'COMPLETED' && !row['rating'], run: row => this.promptAmount(rating => this.api.rateService(row['id'], rating, 'Rated from app')) }
      ],
      amenities: [
      ...
      sublets: [
        ...
        { label: 'Complete', icon: 'task_alt', show: row => this.auth.role() === 'MANAGER' && row['status'] === 'APPROVED', run: row => this.promptText('Guest name', guestName => this.api.completeSublet(row['id'], { guestName, guestPhone: '9000000000', checkInDate: new Date().toISOString().slice(0, 10) })) }
      ]
    };
    return actions[key];
  }
  ...
  private weekLabel(): string {
    const now = new Date();
    return `${now.getFullYear()}-W${Math.ceil((((now.getTime() - new Date(now.getFullYear(), 0, 1).getTime()) / 86400000) + new Date(now.getFullYear(), 0, 1).getDay() + 1) / 7)}`;
  }
  ...
  private promptText(label: string, factory: (value: string) => { subscribe: (handlers: { next: () => void; error: (err: any) => void }) => void }) {
    const value = window.prompt(label);
    if (value) this.mutate(factory(value));
  }

  private promptAmount(factory: (value: number) => { subscribe: (handlers: { next: () => void; error: (err: any) => void }) => void }) {
    const value = Number(window.prompt('Amount / rating'));
    if (Number.isFinite(value) && value > 0) this.mutate(factory(value));
  }
}
